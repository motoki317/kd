  ...
  createEffect(() => {
    ...
    const close = streamGraph(c, ns, {
      snapshot: (g) => {
        ...
        // Read the selection for every snapshot, not only when the outer subscription opened: the
        // watchdog can replace its EventSource and deliver another snapshot through this same closure.
        // Capacity nodes participate because the Nodes view can select a pod outside this namespace.
        const selectionNodes = g.nodes.concat(untrack(capacity)?.nodes ?? [])
        const sel = resolveSelectionOnSnapshot(selectionNodes, untrack(selectedId), pendingSel())

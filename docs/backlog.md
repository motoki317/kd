...
- The drawer's CPU/memory bars now mirror the Nodes capacity view: per resource, a Use bar (live usage) over a Req bar (reserved request) sharing one track — 4 bars total (CPU/Mem × Use/Req). The old single bar mixed a metrics read (fill = usage) with a spec bound (track = allocatable), answering neither "how loaded" nor "how reserved". Now a Node's Use gauges vs total capacity and its Req = Σ requests of every scheduled pod (cluster-wide feed) vs allocatable (Σreq past it = overcommit, marked by an allocatable tick + recolour); a Pod/workload shares its limit as the track, falling back to request, then to its HOST NODE's capacity when unconstrained (so it reads as a fraction of its node, not a fake-full bar). Geometry is a pure tested transform (resourceBars.ts). User-requested; verified live on docker-desktop: node read "USE 0.89/16 cap · REQ 2.05/16 alloc" (Σ pod requests), pod "USE 1m/200m lim · REQ 50m/200m lim", both bars measured on a shared track (c87185a)

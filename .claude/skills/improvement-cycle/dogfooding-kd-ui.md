...
| Exit-0 crashloop (batch script in a Deployment) | `command: ["sh","-c","echo done; exit 0"]` in a Deployment — CrashLoopBackOff in ~40s | the container card explains the contradiction in words ("last exit: Completed — it keeps finishing and restarting; run one-shot work as a Job"), not just the bare facts |
| Saturated HPA | busybox `while :; do :; done` spin (cpu req 10m, lim 200m) under an HPA maxReplicas:2, target 50% utilization — ScalingLimited/TooManyReplicas in ~60s | replicas chip reads "2 · at max" caution-tinted (the "slow workload, green autoscaler" trap); stuck-rollout sibling: progressDeadlineSeconds:30 + a failing readiness probe → headline must say "timed out progressing", not the availability tautology |
| Near-OOM (not yet killed) | `python3 -c "x = bytearray(113*1024*1024); import time; time.sleep(36000)"` under a 128Mi limit (python:3.12-alpine; the interpreter adds ~3Mi — size the bytearray to land 90–95%), beside an idle busybox sidecar | the at-risk container's card alarms in words ("over 90% of its limit, OOM risk") AND its own mem Lim bar reads ~91% full with the Req overshoot hatched, while the calm sidecar's card stays visibly calm (sub-1% fills) — each card gauges its own bars; no pod-level stack/swatches. metrics-server lags ~30–60s after pod start |
